...
import time
import boto3
from pyspark.sql.functions import from_unixtime, from_utc_timestamp, current_timestamp,year, month, sum, current_date, date_sub, round, when
...
start_date = df_transactions.agg({"utc_date": "max"}).collect()[0][0]
...
try:
    ...
    for each in tables_type1:
        ...
        # Delete the Parquet files in the folder
        if len(parquet_files) > 0:
            objects_to_delete = [{'Key': obj_key} for obj_key in parquet_files]
...
df_transactions = df_transactions.selectExpr("*", "explode(items) as product_id")
df_transactions = df_transactions.join(df_products, "product_id")
...
df_transactions = df_transactions.withColumn("transaction_utc_timestamp", from_unixtime("transaction_timestamp"))
...
today = current_date()
df_transactions = df_transactions.filter((year("transaction_utc_timestamp") >= year(date_sub(today, 1095))) & (silver_df_transactions_yearflt.transaction_type=='purchase'))
